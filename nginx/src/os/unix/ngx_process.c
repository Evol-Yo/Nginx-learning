...
ngx_signal_t  signals[] = {
    ...
    { SIGPIPE, "SIGPIPE, SIG_IGN", "", SIG_IGN }, //忽略SIGPIPE信号
...
ngx_signal_handler(int signo)
{
    ...
    case NGX_PROCESS_SINGLE:
        ...
        case SIGCHLD: 
			//如果子进程意外退出，我们设置ngx_reap标志，master进程会调用ngx_reap_children函数
			//处理该进程
            ngx_reap = 1;
            break;
        }
...
ngx_process_get_status(void)
{
    ...
    for ( ;; ) {
		...
		//遍历ngx_processes数组，取得对应进程的信息
        for (i = 0; i < ngx_last_process; i++) {
            if (ngx_processes[i].pid == pid) {
                ngx_processes[i].status = status;	//设置退出状态(包括返回值和信号值)
				//设置exited标志位，master进程在ngx_reap_child函数中进行处理
