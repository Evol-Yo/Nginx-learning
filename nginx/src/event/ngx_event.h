...
typedef struct {
	...
	//启用1个事件，目前事件框架不会调用该方法，大部分事件驱动模块对于该方法的实现与
	//上面的add方法一致
    ngx_int_t  (*enable)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags);
	//禁用1个事件，目前事件框架不会调用该方法，大部分事件驱动模块对于该方法的实现与
	//上面的del方法一致
	...
	//在正常的工作环境中，将通过调用process_events方法来处理事件
...
typedef struct {
	...
	//解析配置项之前，该回调方法用于创建存储配置项的结构体
    void                 *(*create_conf)(ngx_cycle_t *cycle);
	//解析配置项完成之后，该回调函数用以综合处理当前事件模块感兴趣的全部配置项
