...
typedef struct {
	...
	//以下三个参数是必须要设置的,如果不设置将永远无法与上游服务器建立起TCP连接
	//连接上游服务器的超时时间，单位为毫秒
...
struct ngx_http_upstream_s {
	...
	//c)当buffering标志位为0时，buffer缓冲区会被用于反复的接收上游的包体，HTTP模块实现的
	//  input_filter方法需要关注它
    ngx_buf_t                        buffer;
	...
	//那么时在告诉upstream还没有收到完整的响应包头，此时，对于本次upstream请求来说，再次
	...
	//磁盘文件来转发上游的响应包体。当Nginx与上游的网速远大于Nginx与下游客户端间的网速
	//时，让Nginx开辟更多的内存甚至使用磁盘文件来缓存上游的响应包体，这是有意义的，它
	//可以减轻上游服务器的并发压力。当buffering为0时，表示只使用上面的这一个buffer缓冲
	//区来向下游转发响应包体
