...
//调用相应配置块的set函数解析配置块,当配置项的名字为模块名时，这时的set方法就是各个模块的解析方法
//转而解析响应的子模块
static ngx_int_t
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    name = cf->args->elts; //name为解析到的配置项
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        for ( /* void */ ; cmd->name.len; cmd++) {
            ...
            if (ngx_strcmp(name->data, cmd->name.data) != 0) {
                continue; //如果不是相应ngx_command_t数组中的配置项，则跳过
            }


            /* is the directive's location right ? */

			//在解析过程中，判断两个名字相同的配置项是否出错
			...
			//匹配到name配置项所在的ngx_modules_t结构体中的ngx_command_t数组中的元素
            
			conf = NULL;
            if (cmd->type & NGX_DIRECT_CONF) {
                conf = ((void **) cf->ctx)[ngx_modules[i]->index]; //如果是子模块我们取ctx其中一项

            } else if (cmd->type & NGX_MAIN_CONF) {
                conf = &(((void **) cf->ctx)[ngx_modules[i]->index]); //如果是核心模块则我们取ctx
...
ngx_conf_set_num_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	//根据ngx_command_t中的offset偏移量，可以找到结构体中的成员，而对于ngx_onf_set_num_slot而言
	//存储数字必须是ngx_int_t类型
    np = (ngx_int_t *) (p + cmd->offset);

	//在这里可以知道为什么要把使用ngx_conf_set_num_slot方法解析的成员在create_loc_conf等方法中
	//初始化为NGX_CONF_UNSET，否则是会报错的
    if (*np != NGX_CONF_UNSET) {
        return "is duplicate";
    }

	//value指向配置项的参数(value[0]为参数名称，value[1]为将要设置的值)
    value = cf->args->elts;
	//将value转换为整形，并设置到create_loc_conf等方法生成的结构体的相关成员上
    *np = ngx_atoi(value[1].data, value[1].len);
    if (*np == NGX_ERROR) {
        return "invalid number";
    }

	//如果ngx_command_t中的Post已经实现，那么还需要调用post->post_handler方法(用来处理解析到
	//参数后我们需要做的一些工作)
    if (cmd->post) {
        post = cmd->post;
