...
//在执行不重启服务升级Nginx的操作时，老的Nginx进程会通过环境变量“NGINX”来传递需要打开的
//监听端口，新的Nginx进程会通过ngx_add_inherited_sockets方法来使用已经打开的TCP监听端口
static ngx_int_t ngx_add_inherited_sockets(ngx_cycle_t *cycle);

static ngx_int_t ngx_get_options(int argc, char *const *argv);

//用运行Nginx时可能携带的目录参数来初始化cycle，包括初始化运行目录、配置目录，并
//生成完整的nginx.conf配置文件路径
...
ngx_module_t  ngx_core_module = {
    NGX_MODULE_V1,	//包括7个域，其中就有ctx_index、index、version
...
main(int argc, char *const *argv)
{
	...
	//分配1024大小的内存池用于临时init_cycle使用（事实上的大小为1024-sizeof(ngx_pool_t),
	//之所以会创建一个临时的init_cycle，主要是因为我们对该init_cycle的初始化工作会一直有效，
	//即使我们需要重新读取配置文件也是如此，所以我们不需要区分是第一次初始化，还是重新初始化，
	//二者都可以在ngx_init_cycle函数中实现
	...
	//创建并初始化核心模块，解析nginx.conf配置文件（解析过程中，核心模块会创建子模块），
	//打开监听端口，初始化所有模块
