...
//初始化ngx_cycle_t结构体(复制old_cycle中的相关路径、初始化（添加）Nginx目录、初始化打开文件链表、
//初始化（添加）共享内存链表、初始化（复制）listening链表、解析nginx.conf配置文件等)
ngx_cycle_t *
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
	...
	//单独创建一个内存池用于ngx_cycle_t的初始化,这样能够获得很好的局部性
	...
	//复制old_cycle中的配置文件目录、Nginx目录、以及配置文件路径
	...
	//调用所有核心模块的create_conf函数生成存放配置项的结构体,事实上Nginx框架默认只有ngx_core_module定义
	//了ngx_core_module_create_conf函数
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }

		//获取核心模块的ctx上下文结构体(主要为了创建和初始化ngx_core_conf_t结构体)
        module = ngx_modules[i]->ctx;

        if (module->create_conf) {
			//调用ctx->create_conf创建ngx_core_conf_t结构体，并将其成员初始化为NGX_CONF_UNSET
            rv = module->create_conf(cycle);
            if (rv == NULL) {
                ngx_destroy_pool(pool);
                return NULL;
            }
			//将生成的ngx_core_conf_t结构体按照该模块在ngx_modules数组中的下标放在ngx_cycle_t结构体
			//中的conf_ctx数组相应的位置
    ...
    conf.ctx = cycle->conf_ctx;	//令conf.ctx指向创建的所有核心模块的ngx_core_conf_t结构体数组
    conf.cycle = cycle;
    conf.pool = pool;
    conf.log = log;
	...
	//打开cycle->open_files数组中所有记录文件(一般都为日志文件)
    ...
    } else { //if old_cycle->listening.nelts == 0
	...
	//如果我们需要对监听套结字进行配置
    if (!ngx_test_config) {
        ngx_configure_listening_sockets(cycle); //设置监听套结字选项
    }
